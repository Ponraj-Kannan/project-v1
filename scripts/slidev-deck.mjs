...
// Automatically synchronize Supabase questions to slide deck
try {
  const { syncQuestionSlidesToFile } = await import('../api/_lib/supabase.js')
  await syncQuestionSlidesToFile()
} catch (err) {
  // Silent fallback if credentials or dependencies are initializing
}
